...
def jobInfo(request, pandaid=None, batchid=None):
    ...
    if 'computingsite' in job and job['computingsite'] in pq_object_store_paths:
        ...
        if 'log' in file_stats and 'details' in file_stats['log'] and 'lfn' in file_stats['log']['details']:
            if ospath.endswith('/'):
                oslogpath = ospath + file_stats['log']['details']['lfn']
            else:
                oslogpath = ospath + '/' + file_stats['log']['details']['lfn']
    ...
    # extra info for Event Service jobs
    esjobstr = ''
    evtable = []
    ...
    if not is_json_request(request):
        ...
        data = {
            ...
            'files': files,
            'nfiles': len(files),
            'logfile': file_stats['log']['details'] if 'log' in file_stats and 'details' in file_stats['log'] else {},
            'oslogpath': oslogpath,
            'stdout': stdout,
            'stderr': stderr,
            'stdlog': stdlog,
...
def userInfo(request, user=''):
    ...
    if user == '':
        ...
        if request.user.is_authenticated:
            ...
            if user == f"{first_name} {last_name}":
    ...
    if 'view' in request.session['requestParams'] and request.session['requestParams']['view'] == 'classic':
        ...
        values = (
            ...
            'prodsourcelabel', 'specialhandling', 'vo', 'modificationtime', 'pandaid', 'atlasrelease', 'jobsetid', 'processingtype',
            'workinggroup', 'jeditaskid', 'taskid', 'currentpriority', 'creationtime', 'starttime', 'endtime', 'brokerageerrorcode',
            'brokerageerrordiag', 'ddmerrorcode', 'ddmerrordiag', 'exeerrorcode', 'exeerrordiag', 'jobdispatchererrorcode',
            'jobdispatchererrordiag', 'piloterrorcode', 'piloterrordiag', 'superrorcode', 'superrordiag', 'taskbuffererrorcode',
...
def getErrorSummaryForEvents(request):
    ...
    for eventserror in eventsErrors:
        try:
            ...
            eventserror['error_description'] = error_descriptions[
                f"pilot:{str(eventserror['error_code'])}"
            ]['diagnostics'] if f"pilot:{str(eventserror['error_code'])}" in error_descriptions else ''
        except:
...
def errorSummary(request):
    ...
    if not is_json_request(request):
        ...
        savestates = ['finished', 'failed', 'cancelled', ]
        errsBySite = errsBySite[:display_limit]
        if 'jeditaskid' not in request.session['requestParams']:
            sitestates = summary_by_site_overall([site['name'] for site in errsBySite], query['modificationtime__castdate__range'])
        ...
        if not testjobs:
            ...
            for task in taskstatesummary:
                ...
                if 'name' in task:
                    taskstates[taskid]['name'] = task['name']
        ...
        excluded_params = ('display_limit', 'limit', 'hours', 'days')
        ...
        del request.session['TFIRST']
        del request.session['TLAST']
        if 'fields' in request.session['requestParams'] and request.session['requestParams']['fields']:
            fields = request.session['requestParams']['fields'].split(',')
        else:
            fields = ('jobSummary', 'errsByCount', 'errsBySite', 'errsByUser', 'errsByTask' )
        data = {}
...
def getBadEventsForTask(request):
    ...
    for row in evtable:
        ...
        dataitem['ERROR_CODE'] = "{} ({})".format(
            error_desc[f'pilot:{row[1]}']['diagnostics'] if f'pilot:{row[1]}' in error_desc else '',
            str(row[1])
        )
