...
def runTest(x):
    ...
    try:    
        result = req.request(urlToTest, timeout=timeOutForQuery, headers=headers)
...
if __name__ == '__main__':
    ...
    for outout in outputList:
        if outout['status'] != 200:
            ...
            countErrors += 1
    print('All tests passed') if countErrors == 0 else ' -1 status means exceeding time limit'
